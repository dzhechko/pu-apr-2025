 ...
 */
async function compileReport(request) {
    try {
...
`;
        ...
        const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => {
                reject(error_handler_1.createError.timeoutError('OpenAI request timed out after 25 seconds'));
            }, 25000);
        });
...
`
        ...
        if (!reportText) {
            throw error_handler_1.createError.internalServerError('Failed to generate report: empty response');
        }
        ...
        if (error.name === 'AbortError' || error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED') {
            throw error_handler_1.createError.timeoutError('OpenAI request timed out');
        }
 ...
 */
function generateMinimalReport(question) {
