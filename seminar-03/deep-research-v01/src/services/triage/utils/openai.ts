...
import { PlanItem, PlanRequest, PlanResponse } from '../../../shared/types';
 ...
 */
export async function generateResearchPlan(request: PlanRequest): Promise<PlanResponse> {
  try {
...
`;
    ...
    if (!responseText) {
      throw createError.internalServerError('Failed to generate research plan: empty response');
    }

    // Parse the JSON response
    try {
      const plan = JSON.parse(responseText) as PlanResponse;
      
      // Validate the response structure
      if (!plan.plan || !Array.isArray(plan.plan)) {
        throw createError.internalServerError('Invalid research plan format: missing or invalid plan array');
      }
      
      // Validate each plan item
      plan.plan.forEach((item: PlanItem) => {
        if (!item.id || !item.query || !item.domain || !item.priority) {
          throw createError.internalServerError('Invalid research plan item: missing required fields');
        }
      });
      ...
      throw createError.internalServerError('Failed to parse research plan JSON response');
    }
  } catch (error: any) {
